...
def scrape_news():
    ...
    if os.path.exists('busi.txt'):
        os.remove('busi.txt')

    busi_fp = open('busi.txt', 'w', encoding='utf-8')
    for i in range(len(title_arr)):
        busi_fp.writelines(title_arr[i] + '\n')
        busi_fp.writelines(link_arr[i] + '\n')
        busi_fp.writelines(image_arr[i] + '\n')
    busi_fp.close()
